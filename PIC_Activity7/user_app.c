...
{
    static u16 u16Counter = 0;                   //Counter in charge of index for u16Notes array
    static u16 u16Counter2 = 0;                  //Counter in charge of index for u16Breaks array
    static u16 u16Counter3 = 0;                  //Counter in charge of the amount of times userapp is repeated for each note
    u16 u16Notes[] = {C4,0,C4,0,G4,0,G4,0,A4,0,F4,0,A4,0,G4,0,F4,0,F4,0,E4,0,E4,0,D4,0,D4,0,C4, 0}; //Notes to be played
    u16 u16Breaks[] = {N4,RT,N4,RT,N4,RT,N4,RT,N4,RT,N4,RT,N4,RT,N2,RT,N4,RT,N4,RT,N4,RT,N4,RT,N4,RT,N4,RT,N2,RT,N1};    //Length of notes
    if(u16Counter3 < u16Breaks[u16Counter2]){  //Keep playing note till appropriate length of note has been fulfilled
    InterruptTimerXus(u16Notes[u16Counter],1);  //Plays note
    u16Counter3++;                              //Note has been player for another 1ms
    } else {
    u16Counter++;                               //Goes to next index in u16Notes array
    u16Counter2++;                              //Goes to next index in u16Breaks array
    u16Counter3 = 0;                            //Set note duration to having been played for 0ms
    }
    if(u16Counter >29 ){                        //Resets once song has been finished
        u16Counter = 0;
        u16Counter2 = 0;
    }
